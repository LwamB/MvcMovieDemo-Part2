    ...
    public class MoviesController : Controller
    {
        ...
        public IActionResult List(int ratingID = 0)
        {
            ...
            if (ratingID != 0)
            {
                ...
                listMoviesVM.Movies = movieRepository.GetAll().ToList().Where(m => m.RatingID == ratingID).OrderBy(m => m.Title).ToList();
            }
            else
            {

                listMoviesVM.Movies = movieRepository.GetAll().ToList().OrderBy(m => m.Title).ToList();
            }
